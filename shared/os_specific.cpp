...
)
{
    ...
    if (!CreatePipe(&hPipeRead, &hPipeWrite, &saAttr, 0)) {
        log_output("Returning, failed createpipe, error: " + GetLastErrorAsString());
    ...
    BOOL fSuccess = CreateProcessW(NULL, (LPWSTR)cmd, NULL, NULL, TRUE, CREATE_NO_WINDOW, NULL, NULL, &si, &pi);
    if (! fSuccess)
    {
        CloseHandle(hPipeWrite);
        CloseHandle(hPipeRead);
        log_output("Returning, failed create process. error: " + GetLastErrorAsString());
